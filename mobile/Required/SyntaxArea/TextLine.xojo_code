	...
	#tag Method, Flags = &h0, Description = 44726177732074686973206C696E652E
		Sub Paint(storage As SyntaxArea.GapBuffer, g As Graphics, x As Double, y As Double, defaultColor As Color, displayInvisible As Boolean, invisibleColor As Color, selStart As Integer, selLength As Integer, showLeadingSpace As Boolean, indentVisually As Boolean, foldedTrailImage As Picture)
		  ...
		  For i As Integer = 0 To Words.LastIndex
		    ...
		    // Mobile platforms don't support the Graphics.Bold property: Issue 74280.
		    g.Underline = Word.Underline Or underline
		    #If Not (TargetIOS Or TargetAndroid)
		      g.Bold = Word.Bold Or bold
		    #EndIf
		    // Mobile platforms don't support the Graphics.Italic property: Issue 74281.
		    #If Not (TargetIOS Or TargetAndroid)
		      g.Italic = word.Italic Or italic
		    #EndIf
		    // Cache the width.
		    #If TargetIOS Or TargetAndroid
		      If word.LastFont <> g.Font.Name Or word.LastSize <> g.Font.Size Or showInvisible <> displayInvisible Then
		        word.Width = -1
		      End If
		    #Else
		    ...
		    If(word.Type = TYPE_WORD Or word.Type = TYPE_PLACEHOLDER Or displayInvisible) And x + word.Width >= 0 And x < g.Width And y >= 0 And y <= g.Height + g.TextHeight Then
		      ...
		      If word.TYPE = TYPE_PLACEHOLDER Then
		        ...
		        #If TargetIOS Or TargetAndroid
		          g.FillRoundRectangle(x, y - g.Font.Ascent, word.Width, g.TextHeight + 1, g.TextHeight, g.TextHeight)
		        #Else
		        ...
		        #If TargetIOS Or TargetAndroid
		          g.DrawRoundRectangle(x, y - g.Font.Ascent, word.Width, g.TextHeight + 1, g.TextHeight, g.TextHeight)
		        #Else
		    ...
		    #If TargetIOS Or TargetAndroid
		      word.LastFont = g.Font.Name
		      word.LastSize = g.Font.Size
		    #Else
		  ...
		  If folded Then
		    #If TargetIOS Or TargetAndroid
		      g.DrawPicture(foldedTrailImage, x + 3, y - g.Font.Ascent + (g.TextHeight - foldedTrailImage.Graphics.Height) / 2 + 1)
		    #Else
	...
	#tag Method, Flags = &h0, Description = 55736564207768656E207072696E74696E672066726F6D2074686520656469746F722E2044726177732074686973206C696E652E
		Function PrinterPaint(storage As SyntaxArea.GapBuffer, g As Graphics, x As Double, y As Double, w As Integer, defaultColor As Color, displayInvisible As Boolean, wrap As Boolean, indentVisually As Boolean) As Integer
		  ...
		  For i As Integer = 0 To iLimit
		    word = words(i)
		    #If TargetIOS Or TargetAndroid
		      // Color.HighlightColor is missing in Xojo's mobile framework. Issue: 74282.
		      g.DrawingColor = Color.Blue.DarkerColor(50, False)
		    #Else
		    ...
		    #If Not (TargetIOS Or TargetAndroid)
		      // The Xojo mobile framework is missing the Graphics.Bold property.
		      g.Bold = word.Bold Or Bold
		    #EndIf
		    g.Underline = word.Underline Or Underline
		    #If Not (TargetIOS Or TargetAndroid)
		      // The Xojo mobile framework is missing the Graphics.Italic property.
		      g.Italic = word.Italic Or Italic
		    #EndIf
		    ...
		    If word.HasBackgroundColor And word.Type <> word.TYPE_EOL Then
		      ...
		      #If TargetIOS Or TargetAndroid
		        g.FillRectangle(Ceiling(x), y - g.Font.Ascent, Ceiling(word.Width), g.TextHeight + 1)
		      #Else
	...
	#tag Method, Flags = &h0
		Function TextWidth(buffer As SyntaxArea.GapBuffer, g As Graphics, displayInvisible As Boolean, length As Integer = -1) As Single
		  ...
		  If length < 0 Then
		    length = Self.Length
		    #If TargetIOS Or TargetAndroid
		      If Self.LastFont <> g.Font.Name Or Self.LastSize <> g.Font.Size Then width = 0
		    #Else
		  ...
		  #If TargetIOS Or TargetAndroid
		    Self.LastFont = g.Font.Name
		    Self.LastSize = g.Font.Size
		  #Else
		  ...
		  For i As Integer = 0 To u
		    ...
		    #If TargetIOS Or TargetAndroid
		      If word.LastFont <> g.Font.Name Or word.LastSize <> g.Font.Size Then
		        word.Width = -1
		      End If
		    #Else
		    ...
		    If word.Width < 0 Or charsToRead < word.Length Then
		      #If Not (TargetIOS Or TargetAndroid)
		        // The mobile framework lacks these properties.
		        g.Bold = word.Bold
