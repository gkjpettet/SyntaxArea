...
#tag WindowCode
	...
	#tag Constant, Name = EXAMPLE_WREN, Type = String, Dynamic = False, Default = \"/* This\n   is\n   a\n   multi-line\n   comment. */\n\n// Single line comment.\n\nSystem.print(\"Math %(3 + 4 * 5) is really %(something)!\") //> Math 23 is fun!\nSystem.print(\"wow %((1..3).map {|n| n * n}.join())\") //> wow 149\n\"\"\"hi there\"\"\"\n\n\"\\0\" // The NUL byte: 0.\n\"\\\"\" // A double quote character.\n\"\\\\\" // A backslash.\n\"\\%\" // A percent sign.\n\"\\a\" // Alarm beep. (Who uses this\?)\n\"\\b\" // Backspace.\n\"\\e\" // ESC character.\n\"\\f\" // Formfeed.\n\"\\n\" // Newline.\n\"\\r\" // Carriage return.\n\"\\t\" // Tab.\n\"\\v\" // Vertical tab.\n\"\\x48\" // Hex escapes.\n\"\\u0041\" // Unicode escapes.\n\"\\U0001F64A\" // Extended Unicode escapes.\n\n// Identifiers & fields\nhi\ncamelCase\nPascalCase\n_under_score\nabc123\nALL_CAPS\n\n// Numbers\n0\n1234\n-5678\n3.14159\n1.0\n-12.34\n0.0314159e02\n0.0314159e+02\n314.159e-02\n0xcaffe2\n\nclass Unicorn {\n  prance() {\n    System.print(\"The unicorn prances in a fancy manner!\")\n  }\n}", Scope = Protected, Description = 4578616D706C65204320636F64652E
	#tag EndConstant
	...
	#tag Constant, Name = EXAMPLE_XOJO, Type = String, Dynamic = False, Default = \"/// Loads an XML syntax definition.\n\nVar xml As XmlDocument\nVar root\x2C node As XMLNode\nVar context As SyntaxArea.HighlightContext\nVar symbol As SyntaxArea.SymbolsDefinition\nVar i\x2C j As Integer\n\nTry\n  xml \x3D New XmlDocument\n  xml.LoadXml(data)\n  \n  root \x3D xml.Child(0)\n  If root.Name <> \"highlightDefinition\" Then\n    Return False\n  End If\n  If Val(root.GetAttribute(\"version\")) > VERSION Then\n    Return False\n  End If\n  \n  Var lastStartRule As RegEx\n  \n  For i \x3D 0 To root.ChildCount - 1\n    node \x3D root.Child(i)\n    Select Case node.Name\n    Case \"name\"\n      // Syntax name.\n      Name \x3D node.FirstChild.Value\n      \n    Case \"blockStartMarker\"\n      If lastStartRule <> Nil Then\n        // Error: There\'s still an unfinished start rule open.\n        Return False\n      End\n      Var newstate As XmlAttribute \x3D node.GetAttributeNode(\"newstate\")\n      Var newstateValue As String\n      If newstate <> Nil Then newstateValue \x3D newstate.Value\n      Var cond As String \x3D node.GetAttribute(\"condition\")\n      \n      Var re As New RegEx\n      re.SearchPattern \x3D node.FirstChild.Value\n      lastStartRule \x3D re\n      \n      Var blockStartDefs() As BlockStartDefinition\n      Var tmpVariant As Variant \x3D BlockStartDef.Lookup(cond\x2C Nil)\n      If Not tmpVariant.IsArray Then\n        BlockStartDef.Value(cond) \x3D blockStartDefs\n      Else\n        blockStartDefs \x3D tmpVariant\n      End If\n      \n      Var state As New BlockState(newState <> Nil\x2C newstateValue)\n      Var blockData As New BlockStartData(node.GetAttribute(\"indent\").Val\x2C state)\n      blockStartDefs.Add(New BlockStartDefinition(re\x2C blockData))\n      \n    Case \"blockEndMarker\"\n      If lastStartRule \x3D Nil Then\n        // Error: End rule without start rule.\n        Return False\n      End\n      Var newstate As XmlAttribute \x3D node.GetAttributeNode(\"newstate\")\n      Var newstateValue As String\n      If newstate <> Nil Then newstateValue \x3D newstate.Value\n      Var cond As String \x3D node.GetAttribute(\"condition\")\n      Var re As New RegEx\n      re.SearchPattern \x3D node.FirstChild.Value\n      \n      Var blockEndDefs() As BlockEndDefinition\n      Var tmpVariant As Variant \x3D BlockEndDef.Lookup(cond\x2C Nil)\n      If Not tmpVariant.IsArray Then\n        BlockEndDef.Value(cond) \x3D blockEndDefs\n      Else\n        blockEndDefs \x3D tmpVariant\n      End If\n      \n      Var state As New BlockState(newState <> Nil\x2C newstateValue)\n      Var blockData As New BlockEndData(lastStartRule\x2C state)\n      blockEndDefs.Add(New BlockEndDefinition(re\x2C blockData))\n      \n      lastStartRule \x3D Nil\n      \n    Case \"lineContinuationMarker\"\n      // `indent` is the number of indentations.\n      Var re As New RegEx\n      re.SearchPattern \x3D node.FirstChild.Value\n      LineContinuationDef.Value(re) \x3D Val(node.GetAttribute(\"indent\"))\n      \n    Case \"symbols\"\n      For j \x3D 0 To node.ChildCount - 1\n        Var child As XmlNode \x3D node.Child(j)\n        If child.Name \x3D \"symbol\" Then\n          symbol \x3D New SyntaxArea.SymbolsDefinition\n          symbol.loadFromXmlNode(child)\n          AddSymbol(symbol)\n        End If\n      Next j\n      \n    Case \"placeholders\"\n      PlaceholderContextDef \x3D New SyntaxArea.HighlightContext(Self.Owner\x2C False\x2C False\x2C Self)\n      PlaceholderContextDef.EntryRegEx \x3D node.FirstChild.Value\n      PlaceholderContextDef.IsPlaceholder \x3D True\n      PlaceholderContextDef.Name \x3D \"placeholder\"\n      \n      // Enabled\?\n      Var tmp As String\n      tmp \x3D node.GetAttribute(\"enabled\")\n      placeholderContextDef.Enabled \x3D tmp <> \"false\"\n      \n      Self.AddContext(PlaceholderContextDef)\n      \n    Case \"contexts\"\n      // Contexts.\n      CaseSensitive \x3D YN2Bool(node.GetAttribute(\"caseSensitive\"))\n      For j \x3D 0 To node.ChildCount-1\n        // Ignore XML comments.\n        If node.Child(j) IsA XMLComment Then Continue\n        context \x3D New SyntaxArea.HighlightContext(Self.Owner\x2C CaseSensitive\x2C Self)\n        context.LoadFromXmlNode(node.Child(j))\n        AddContext(context)\n      Next j\n    End Select\n  Next i\n  \n  // Add a blank space context.\n  Self.AddBlankSpaceContext\n  \n  // Finalise self references.\n  If ContextsToSelfReference.KeyCount > 0 Then\n    For Each entry As DictionaryEntry In ContextsToSelfReference\n      Var hc As HighlightContext \x3D entry.Key\n      For Each c As SyntaxArea.HighlightContext In Self.Contexts\n        hc.AddSubContext(c)\n      Next c\n    Next entry\n  End If\n  \n  Return True\nCatch e As RuntimeException\n  Return False\nEnd Try\n", Scope = Protected, Description = 4578616D706C6520586F6A6F20636F64652E
	#tag EndConstant
