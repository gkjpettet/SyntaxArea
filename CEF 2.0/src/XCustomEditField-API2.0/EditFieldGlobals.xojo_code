...
Protected Module EditFieldGlobals
	...
	#tag Method, Flags = &h0
		Function DarkerColor(extends forColor as Color, offset as integer, adjustForDarkMode as Boolean) As Color
		  ...
		  if adjustForDarkMode and IsDarkMode then
		    Return Color.RGB( min(forColor.Red + offset, 255), min(forColor.green + offset, 255), min(forColor.blue + offset, 255)) // darker
		  else
	...
	#tag Method, Flags = &h0
		Function InvertColor(extends input as Color) As color
		  Return Color.RGB(255 - input.Red, 255 - input.Green, 255 - input.Blue)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function LighterColor(extends forColor as color, offset as integer, adjustForDarkMode as Boolean) As color
		  //get a darker color for the given color.
		  if adjustForDarkMode and IsDarkMode then
		    Return Color.RGB( max(forColor.Red - offset, 0), max(forColor.green - offset, 0), max(forColor.blue - offset, 0)) // lighter
		  else
	...
	#tag Constant, Name = CEF_VERSION, Type = String, Dynamic = False, Default = \"2.0.0", Scope = Protected
	#tag EndConstant
