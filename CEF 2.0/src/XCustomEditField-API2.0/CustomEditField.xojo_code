...
Implements MessageReceiver
	...
	#tag Event
		Sub Closing()
		  ...
		  if mHighlightTimer <> nil then
		    mHighlightTimer.RunMode = Timer.RunModes.Off //0
		    mHighlightTimer.Enabled = false
		    mHighlightTimer = nil
		  end if
		  
		  if mRedrawTimer <> nil then
		    mRedrawTimer.RunMode = Timer.RunModes.Off //0
	...
	#tag Event
		Sub MenuBarSelected()
		  'self.EditCopy.Enabled = me.selLength > 0
		  'self.EditClear.Enabled = EditCopy.Enabled
		  'self.EditCut.Enabled = EditCopy.Enabled
		  '
	...
	#tag Event
		Sub MouseMove(x As Integer, y As Integer)
		  ...
		  s = s.ReplaceAll( "=", " ")
		  s = s.ReplaceAll( "/", " ")
		  //DemoWindow.Title = str(Me.CharPosAtXY(x,y))
	...
	#tag Method, Flags = &h1
		Protected Sub AutocompleteEOL()
		  ...
		  if maxIndex >0 then //more than 1 option
		    ...
		    trailingSuggestion = longestCommonPrefix.Right(longestCommonPrefix.Length - currentPathComponent.Length) + "…"
		  ...
		  if option <> "" then
		    if SelLength > 0 then
		      ...
		      if AutocompleteAppliesStandardCase then
		        ...
		        private_replace(CaretPos - CurrentAutocompleteOptions.CurrentPathComponent.Length, CurrentAutocompleteOptions.CurrentPathComponent.Length, option)
		      else
	...
	#tag Method, Flags = &h0
		Sub Constructor()
		  ...
		  Super.Constructor //DesktopUIControl
		  ignoreRepaint = true
	...
	#tag Method, Flags = &h0
		Function CustomEditFieldPrinter(printerGraphics as graphics) As CustomEditFieldPrinter
		  printerGraphics.FontName = TextFont
		  printerGraphics.FontSize = FontSize
		...
		Function DocumentSymbols() As DocumentSymbol()
		  ...
		  for Each line in CurrentDocumentSymbols.keys
		    ...
		    for each Symbol in line.LineSymbols.Values
		      ...
		      tmp.Add( new DocumentSymbol(symbol.Name, line.offset + symbol.Offset, symbol.Type) ) //with the correct offset!
		    next
		  next
	...
	#tag Method, Flags = &h21
		Private Sub drawContents(gr as graphics, parentWindow as DesktopWindow)
		      ...
		      if displayLineNumbers then
		        ...
		        if CaretLine = lineIdx then
		          ...
		          gg.DrawingColor = EditFieldGlobals.AdjustColorForDarkMode (&c000000)
		            ...
		            gg.DrawingColor =  EditFieldGlobals.AdjustColorForDarkMode (Color.RGB(rnd * 255, rnd * 255, rnd * 255)) 'GutterBackgroundColor
		          #else
		            gg.DrawingColor =  GutterBackgroundColor
		          #endif
		          gg.FillRectangle 0,sy - g.TextHeight,LineNumOffset - 1 - FoldingOffset, TextHeight
		  ...
		  'tmpPic.Graphics.AntiAliasMode = Graphics.AntiAliasModes.HighQuality
		  'g = tmpPic.Graphics
		  ...
		  'gb.FillRectangle 0, 0, blockPicture.Width, blockPicture.Height
		  '
		  ...
		  'gb.DrawRoundRectangle MouseOverBlock.value("x"), MouseOverBlock.Value("y"), MouseOverBlock.Value("w"), MouseOverBlock.Value("h"), 6,6
		  '
		  'gb.ForeColor = EditFieldGlobals.AdjustColorForDarkMode (&C999999)
		  'gb.DrawRoundRectangle MouseOverBlock.value("x"), MouseOverBlock.Value("y"), MouseOverBlock.Value("w"), MouseOverBlock.Value("h"), 8,8
		  'gb.DrawRoundRectangle MouseOverBlock.value("x")+1, MouseOverBlock.Value("y")+1, MouseOverBlock.Value("w")-2, MouseOverBlock.Value("h")-2, 10,10
		  '
		  'gb.ForeColor = EditFieldGlobals.AdjustColorForDarkMode (&C888888)
		  'gb.DrawRoundRectangle MouseOverBlock.value("x"), MouseOverBlock.Value("y"), MouseOverBlock.Value("w"), MouseOverBlock.Value("h"), 10,10
		  'gb.DrawRoundRectangle MouseOverBlock.value("x")+1, MouseOverBlock.Value("y")+1, MouseOverBlock.Value("w")-2, MouseOverBlock.Value("h")-2, 8,8
		  '
		  ...
		  '
		  'blockPicture.mask.Graphics.DrawingColor = EditFieldGlobals.AdjustColorForDarkMode (&cAAAAAA)
		  'blockPicture.mask.Graphics.FillRectangle 0, 0, blockPicture.Width, blockPicture.Height
		  '
		  'blockPicture.mask.Graphics.DrawingColor = EditFieldGlobals.AdjustColorForDarkMode (&cffffff)
		  ...
		  '
		  'blockPicture.mask.Graphics.DrawingColor = EditFieldGlobals.AdjustColorForDarkMode (&C888888)
		  'blockPicture.mask.Graphics.PenSize = 2
		  'blockPicture.mask.Graphics.PenHeight = 2
		  'blockPicture.Mask.Graphics.DrawRoundRectangle MouseOverBlock.value("x"), MouseOverBlock.Value("y"), MouseOverBlock.Value("w"), MouseOverBlock.Value("h"), 10, 10
		  '
	...
	#tag Method, Flags = &h1
		Protected Function fetchAutocompleteOptions() As boolean
		  ...
		  //TODO: Come back and remove this comment...
		...
		Protected Sub getFieldXY(byref locx as Double, byref locy as Double)
		  ...
		  while true
		    ...
		    elseif Container isa DesktopWindow then
		      ...
		      //Fixed by Matthew Combatti - 2023! Wallah :-)
	...
	#tag Method, Flags = &h1
		Protected Function HandleKeyDown(key as string) As boolean
		  ...
		  Else
		    ...
		    if not typing or CurrentEventID = 0  or System.Ticks > CurrentEventID + (60 * UNDO_EVT_BLOCK_SECS) then CurrentEventID = System.Ticks
		    typing = true
		  ...
		  #if DebugBuild
		    keyDownTime = System.Microseconds - keyDownTime
		  #endif
	...
	#tag Method, Flags = &h0
		Function HighLighterTaskBusy() As Boolean
		  ...
		  return mHighlighter.ThreadState <> Thread.ThreadStates.NotRunning
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h1
		Protected Sub PaintHighlightedBlock(g as graphics)
		  ...
		  g.PenSize = 2
		  g.DrawingColor = EditFieldGlobals.AdjustColorForDarkMode (&c4444FF)
	...
	#tag Method, Flags = &h0
		Sub private_replace(offset as integer, length as integer, TheText as string, alwaysMarkChanged as Boolean = true, eventID as integer = -1, keepSelection as Boolean = false, beSilent as Boolean = false)
		  // This method is used internally by the control, and externally by the undo mechanism, you shouldn't use it directly, use instead selstart and seltext.
		  ...
		  //modify buffer and rescan lines
		  TextStorage.replace(offset, length, TheText)
		  lines.replace(offset, length, TheText, alwaysMarkChanged)
		  ...
		  if keepSelection then
		    ...
		    changeSelection(offset + TheText.Length, 0)
		  end
		  ...
		  if not UndoMgr.isUndoing then
		    ...
		    RaiseEvent TextInserted(offset, TheText)
		    ...
		    if TheText.Length > 1 or not IsBlockChar(TheText) or not HighlightMatchingBrackets then Return
		    
		    //if so, highlight it
		    if BLOCK_CLOSE_CHARS.IndexOf(TheText) > -1 then
		      HighlightOpeningBlock(TheText, offset)
		    Else
		      HighlightClosingBlock(TheText, offset)
		    end if
		  end if
	...
	#tag Method, Flags = &h21
		Private Sub RefreshBlocked(eraseBackground As Boolean = True)
	...
	#tag ComputedProperty, Flags = &h0
		...
		#tag Getter
			Get
			  if Thread.Current = nil then // this check prevents "ThreadAccessingUIException" when called from a thread
	...
	#tag Constant, Name = RegEXURL, Type = String, Dynamic = False, Default = \"(\?x) # FREE SPACING\r(\?i-U) # Case-insensitive\x2C greedy\r\r# Define the prefix\r(\?(DEFINE)(\?<prefix>[A-Z]{3\x2C}://))\r# Define a valid URL character\r(\?(DEFINE)(\?<valid>[A-Z0-9\\-_~:/\?\\#[\\]@!$&\'()*+;\x3D.\x2C%]))\r\r# START\r\\b # Word boundary\r(\?: # Non-capturing group\r(\?<\x3D\\<)(\?&prefix)(\?&valid)+(\?\x3D\\>) # Anything between angle-brackets\r| # OR\r(\?<\x3D\\[)(\?&prefix)(\?&valid)+(\?\x3D\\]) # Anything between square-brackets\r| # OR\r(\?<\x3D\\{)(\?&prefix)(\?&valid)+(\?\x3D\\}) # Anything between curly-brackets\r| # OR\r(\?&prefix)(\?&valid)+(\?<![\\.\x2C]) # Can\'t end on a dot or comma\r) # End non-capturing group", Scope = Private
	#tag EndConstant
