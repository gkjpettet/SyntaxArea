...
Protected Class HighlightContext
	...
	#tag Method, Flags = &h0
		Function ContextSearchPattern() As String
		  ...
		  if keywords.LastIndex > -1 then
		    ...
		    next
		    _contextPattern = _contextPattern.Left(_contextPattern.Length-1) + ")\b"
		  ...
		  if regexes.LastIndex >-1 then
		    ...
		    next
		    _contextPattern = _contextPattern.Left(_contextPattern.Length-1) + ")"
		    Return _contextPattern
		  end if
	...
	#tag Method, Flags = &h0
		Function Highlight(TheText as string, subExpression as string, position as integer, positionB as integer, scanner as regex, tokens() as textsegment, placeholders() as TextPlaceholder) As boolean
		  ...
		  if StartRegEx.trim<>"" and mEndRegEx<>nil then
		    //find end...
		      ...
		      match = mEndRegex.Search(TheText, positionB + subExpression.Lenb) //fix, added .lenb to support utf correctly
		      scanner.SearchStartPosition = mEndRegex.SearchStartPosition
		    #endif
		    
		    //find the subExpression
		    if match<>nil then
		      subExpression = TheText.LeftBytes(match.SubExpressionStartB(0)+match.SubExpressionString(0).Bytes)
		      subExpression = subExpression.Right(subExpression.Length-position)
		    else
		      //no match? then use the rest of the String...
		      subExpression = TheText.Right(TheText.Length-position)
		      scanner.SearchStartPosition = TheText.Bytes //fix, added .lenb to support utf correctly
		      scanNextLine = true
		    end if
		  end if
		  ...
		  if substring = "" then
		    ...
		    case " "
		      tokens.Add(new TextSegment(position, 1, TextSegment.TYPE_SPACE, HighlightColor, BackgroundColor))
		      
		    case chr(9)
		      tokens.Add(new TextSegment(position, 1, TextSegment.TYPE_TAB, HighlightColor, BackgroundColor))
		      
		    case chr(10), chr(13), chr(13) + chr(10)
		      tokens.Add(new TextSegment(position, subExpression.Length, TextSegment.TYPE_EOL, HighlightColor, BackgroundColor))
		      
		    else
		      if subExpression.Length > 0 then _
		      tokens.Add(new TextSegment(position, subExpression.Length, TextSegment.TYPE_WORD, HighlightColor, BackgroundColor, bold, italic, underline))
		    ...
		    while match<>nil
		      ...
		      tokens.Add(new TextSegment(startPos + position, charPos - startPos, TextSegment.TYPE_WORD, HighlightColor, BackgroundColor, bold, italic, underline))
		    ...
		    tokens.Add(new TextSegment(startPos + position, subExpression.Length - startPos, TextSegment.TYPE_WORD, HighlightColor, BackgroundColor, bold, italic, underline))
		  end if
