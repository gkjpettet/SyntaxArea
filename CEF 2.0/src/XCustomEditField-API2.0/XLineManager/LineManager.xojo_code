...
Class LineManager
	#tag Method, Flags = &h1
		Protected Sub appendLine(segment as textline)
		  ...
		  //mark it as changed
		  'NotifyLineChangedRange(lines.LastIndex, 1)
		  ...
		  if sText <> "" then
		    ...
		    for each sLine as string in ars
		      ...
		      if c = 13 then
		        ...
		        if sText.Middle( iOffsetB, EOLlen ).Asc = 10 then //windows
		          iLineLen = iLineLen + 2
	...
	#tag Method, Flags = &h0
		Sub replace(offset as integer, length as integer, TheText as string, alwaysMarkDirty as Boolean)
		  ...
		  if line = nil then
		    // ignore
		  elseif length = 0 and TheText.Length = 1 and TheText <> lineEnding then //simple keystroke
		  ...
		  elseif length = 1 and TheText.Length = 0 then //simple delete
		    RemoveLineSymbols(lineNumber, lineNumber)
