...
Class GapBuffer
	...
	#tag Method, Flags = &h0
		Sub replace(index as integer, length as integer, text as string)
		  ...
		  //add the text
		  buffer.StringValue(index, text.Length) = text
		  gapStart = gapStart + text.Length
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub setText(TheText as string)
		  ...
		  if TheText.Encoding=nil then //Text without encoding cannot be converted and will cause a break in MemoryBlockStorageWide.StringValue
		    raise new TextHasNoEncodingException
		  end if
		  
		  TheText = TheText.ConvertEncoding(EditFieldGlobals.InternalEncoding) ' make sure it's not in UTF-16
		  buffer.Size = TheText.Length
		  buffer.StringValue(0, TheText.Length) = TheText
		  gapStart = TheText.Length/2
		  gapEnd = gapStart
		End Sub
	#tag EndMethod
